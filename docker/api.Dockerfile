...
# Invoke uvicorn as a Python module — bypasses the wrapper script whose shebang
# is baked with the builder-stage venv path (/app/backend/.venv) and would
# `ENOENT` when run from this stage's /app/.venv.
